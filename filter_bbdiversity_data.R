...
##########################################################################################
##########################################################################################
# Filter the data used for the paper - THIS IS A STANDARD FILE ACROSS ALL BUYBACK PAPERS
##########################################################################################
##########################################################################################
...
to_remove = which(
  ...
  is.na(BUYBACK_DATA_PROJECT$BEME_used) | 
    is.na(BUYBACK_DATA_PROJECT$Performance_used) | 
    is.na(BUYBACK_DATA_PROJECT$Size_used) | 
    is.na(BUYBACK_DATA_PROJECT$DATASET$CRSP$pre_vol_Score) |
    is.na(BUYBACK_DATA_PROJECT$DATASET$CRSP$IVOL_score) |
    is.na(BUYBACK_DATA_PROJECT$DATASET$CRSP$Rsq_score) |
    is.na(BUYBACK_DATA_PROJECT$DATASET$CRSP$Market.Cap) 
)
...
to_remove = which(
  # just in alphabetic order not to forget any    
  is.na(ISSUERS_DATA_PROJECT$BEME_used) | 
    is.na(ISSUERS_DATA_PROJECT$Performance_used) | 
    is.na(ISSUERS_DATA_PROJECT$Size_used) | 
    is.na(ISSUERS_DATA_PROJECT$DATASET$CRSP$pre_vol_Score) |
    is.na(ISSUERS_DATA_PROJECT$DATASET$CRSP$IVOL_score) |
    is.na(ISSUERS_DATA_PROJECT$DATASET$CRSP$Rsq_score) |
    is.na(ISSUERS_DATA_PROJECT$DATASET$CRSP$Market.Cap) 
)
if (length(to_remove) > 0){
  # just in alphabetic order not to forget any    
...
# Buybacks first
events = BUYBACK_DATA_PROJECT$DATASET
...
BUYBACK_DATA_PROJECT$cleanupBIZ = cleanup

# Issuers now
events = ISSUERS_DATA_PROJECT$DATASET
No_filter = rep(T,length(events$SDC$Event.Date))
Basic_filter =  No_filter # !is.na(events$SDC$ME_quantile) & !is.na(events$SDC$BEME_quantile)  
Period_filter <- (as.Date(events$SDC$Event.Date) >= First) & (as.Date(events$SDC$Event.Date) <= Last)
#Penny_stock_filter = ifelse(events$SDC$Event.Date < "1995-01-01", scrub(events$SDC$Closing.Price) >= penny_stock_price_old, scrub(events$SDC$Closing.Price) >= penny_stock_price_recent) 
...
cleanup$Market_cap_filter <- sum(!Market_cap_filter)
cleanup$EventSize_filter <- sum(!EventSize_filter)
ISSUERS_DATA_PROJECT$cleanupBIZ = cleanup

###### Check time order of events
ordered_events = sort(as.numeric(BUYBACK_DATA_PROJECT$DATASET$SDC$Event.Date),index.return = T)$ix
if (length(unique(diff(ordered_events)))!=1)
  stop("The time order was messed up somewhere for buybacks")
rm("ordered_events")
ordered_events = sort(as.numeric(ISSUERS_DATA_PROJECT$DATASET$SDC$Event.Date),index.return = T)$ix
if (length(unique(diff(ordered_events)))!=1)
...
if (remove_financials_utilities){
  ...
  # Buybacks first
  Industry_filter = BUYBACK_DATA_PROJECT$DATASET$SDC$Industry %in% INDUSTRY_USED 
  ...
  BUYBACK_DATA_PROJECT$cleanupBIZ$Industry_filter = sum(!Industry_filter)
  
  # Issuers now
  Industry_filter = ISSUERS_DATA_PROJECT$DATASET$SDC$Industry %in% INDUSTRY_USED 
  to_remove = which(!Industry_filter)
  if (length(to_remove) > 0){
    # just in alphabetic order not to forget any    
    ISSUERS_DATA_PROJECT$BEME_used <- ISSUERS_DATA_PROJECT$BEME_used[-to_remove]
  ...
  ISSUERS_DATA_PROJECT$cleanupBIZ$Industry_filter = sum(!Industry_filter)
...
if (remove_missing_permnosV2){
  ...
  # Buybacks first
  to_remove = which(is.na(BUYBACK_DATA_PROJECT$DATASET$SDC$permnoV2))
  if (length(to_remove) > 0){
    # just in alphabetic order not to forget any    
    BUYBACK_DATA_PROJECT$BEME_used <- BUYBACK_DATA_PROJECT$BEME_used[-to_remove]
    BUYBACK_DATA_PROJECT$Performance_used <- BUYBACK_DATA_PROJECT$Performance_used[-to_remove]
  ...
  BUYBACK_DATA_PROJECT$cleanupNoPermno = BUYBACK_DATA_PROJECT$cleanupNoPermno + length(to_remove)
  
  # Issuers now
  to_remove = which(is.na(ISSUERS_DATA_PROJECT$DATASET$SDC$permnoV2))
  if (length(to_remove) > 0){
    # just in alphabetic order not to forget any    
    ISSUERS_DATA_PROJECT$BEME_used <- ISSUERS_DATA_PROJECT$BEME_used[-to_remove]
    ISSUERS_DATA_PROJECT$Performance_used <- ISSUERS_DATA_PROJECT$Performance_used[-to_remove]
  ...
  ISSUERS_DATA_PROJECT$cleanupNoPermno = ISSUERS_DATA_PROJECT$cleanupNoPermno + length(to_remove)
...
if (remove_CRSP_minor_markets){
  ...
  # Buybacks first
  to_remove = which(!(BUYBACK_DATA_PROJECT$DATASET$CRSP$exchange %in% major_markets_crsp))
  if (length(to_remove) > 0){
    # just in alphabetic order not to forget any    
    BUYBACK_DATA_PROJECT$BEME_used <- BUYBACK_DATA_PROJECT$BEME_used[-to_remove]
    BUYBACK_DATA_PROJECT$Performance_used <- BUYBACK_DATA_PROJECT$Performance_used[-to_remove]
  ...
  BUYBACK_DATA_PROJECT$cleanupBIZ$major_markets_only = BUYBACK_DATA_PROJECT$cleanupBIZ$major_markets_only + length(to_remove)
  
  # Issuers now
  to_remove = which(!(ISSUERS_DATA_PROJECT$DATASET$CRSP$exchange %in% major_markets_crsp))
  if (length(to_remove) > 0){
    # just in alphabetic order not to forget any    
    ISSUERS_DATA_PROJECT$BEME_used <- ISSUERS_DATA_PROJECT$BEME_used[-to_remove]
    ISSUERS_DATA_PROJECT$Performance_used <- ISSUERS_DATA_PROJECT$Performance_used[-to_remove]
  ...
  ISSUERS_DATA_PROJECT$cleanupBIZ$major_markets_only = ISSUERS_DATA_PROJECT$cleanupBIZ$major_markets_only + length(to_remove)
